...
function Hui_admin_tab(obj) {
    ...
    if (!bStop) {
        creatIframe(_href, _titleName, $(obj).attr('data-nav'),$(obj).attr('data-icon'));
        min_titleList();
    }
    else {
...
function creatIframe(href, titleName, nav, titleIcon) {
    var topWindow = $(window.parent.document);
    var show_nav = topWindow.find('#min_title_list');
    show_nav.find('li').removeClass("active");
    var iframe_box = topWindow.find('#iframe_box');
    show_nav.append('<li class="active"><i class="Hui-iconfont mr-5">'+titleIcon+'</i><span data-href="' + href + '">' + titleName + '</span><i class="Hui-iconfont ml-5 mb-5">&#xe60b;</i></li>');
    var taballwidth = 0,
    ...
    iframeBox.hide();
    var ts = get_random();
    iframe_box.append('<div class="show_iframe"><div class="loading"><div class="loading-box"><span class="loding-text">努力加载中...</span></div></div><iframe frameborder="0" src=' + href + ' id=' + ts + ' name="' + ts + '"></iframe></div>');
